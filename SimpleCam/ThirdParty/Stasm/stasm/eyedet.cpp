    ...
    const char*    datadir)    // in
{
    if (need_eyes)
    {
        ...
        OpenDetector(leye_det_g,  "haarcascade_lefteye_2splits.xml",  datadir);
        OpenDetector(reye_det_g,  "haarcascade_righteye_2splits.xml", datadir);
		printf(" OpenDetector eye \n");
    }
	if (need_mouth) {
        OpenDetector(mouth_det_g,  "haarcascade_mcs_mouth.xml", datadir);
		printf(" OpenDetector mouth \n");
	}
}
